...
namespace WebDemo.DAL.WebDemo
{
	...
	public partial class FreezeDayLog
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("MeterID", "FreezeDayLog"); 
		}
		...
		public bool Exists(int MeterID,string FreezeDate,DateTime ReadDate,int ID)
		{
			...
			strSql.Append("select count(1) from FreezeDayLog");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(WebDemo.Model.WebDemo.FreezeDayLog model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into FreezeDayLog(");
			strSql.Append("MeterID,FreezeDate,FreezeNumber,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,ValveState,OldFreezeNumber,Megabit,PriceSetNo,CollectorReadDate,Remnant)");
			...
			parameters[15].Value = model.PriceSetNo;
			parameters[16].Value = model.CollectorReadDate;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(WebDemo.Model.WebDemo.FreezeDayLog model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update FreezeDayLog set ");
			...
			parameters[10].Value = model.OldFreezeNumber;
			parameters[11].Value = model.Megabit;
			parameters[12].Value = model.PriceSetNo;
			parameters[13].Value = model.CollectorReadDate;
			parameters[14].Value = model.Remnant;
			parameters[15].Value = model.ID;
			parameters[16].Value = model.MeterID;
			parameters[17].Value = model.FreezeDate;
			parameters[18].Value = model.ReadDate;
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from FreezeDayLog ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebDemo.Model.WebDemo.FreezeDayLog GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,MeterID,FreezeDate,FreezeNumber,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,ValveState,OldFreezeNumber,Megabit,PriceSetNo,CollectorReadDate,Remnant from FreezeDayLog ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebDemo.Model.WebDemo.FreezeDayLog DataRowToModel(DataRow row)
		{
			WebDemo.Model.WebDemo.FreezeDayLog model=new WebDemo.Model.WebDemo.FreezeDayLog();
			if (row != null)
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,MeterID,FreezeDate,FreezeNumber,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,ValveState,OldFreezeNumber,Megabit,PriceSetNo,CollectorReadDate,Remnant ");
			strSql.Append(" FROM FreezeDayLog ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,MeterID,FreezeDate,FreezeNumber,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,ValveState,OldFreezeNumber,Megabit,PriceSetNo,CollectorReadDate,Remnant ");
			strSql.Append(" FROM FreezeDayLog ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from FreezeDayLog T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "FreezeDayLog";
