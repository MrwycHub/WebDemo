...
namespace WebDemo.DAL.WebDemo
{
	...
	public partial class ReadNowData
	{
		...
		public int GetMaxId()
		{
		return DbHelperSQL.GetMaxID("ID", "ReadNowData"); 
		}
		...
		public bool Exists(int ID)
		{
			...
			strSql.Append("select count(1) from ReadNowData");
		...
		/// 增加一条数据
		/// </summary>
		public int Add(WebDemo.Model.WebDemo.ReadNowData model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into ReadNowData(");
			strSql.Append("MeterID,ReadNumber,ValveState,AttackState,Voltage,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,OldFreezeNumber,Megabit,UseState,ControlValveDate,SyncFlag,PriceSetNo,CollectorReadDate,MeterStatusString,Remnant)");
			...
			SqlParameter[] parameters = {
					...
					new SqlParameter("@ControlValveDate", SqlDbType.DateTime),
			...
			parameters[17].Value = model.ControlValveDate;
			parameters[18].Value = model.SyncFlag;
			parameters[19].Value = model.PriceSetNo;
			parameters[20].Value = model.CollectorReadDate;
			parameters[21].Value = model.MeterStatusString;
			parameters[22].Value = model.Remnant;
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(WebDemo.Model.WebDemo.ReadNowData model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update ReadNowData set ");
			...
			strSql.Append("ControlValveDate=@ControlValveDate,");
		...
		public bool Delete(int ID)
		{
			...
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from ReadNowData ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebDemo.Model.WebDemo.ReadNowData GetModel(int ID)
		{
			
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 ID,MeterID,ReadNumber,ValveState,AttackState,Voltage,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,OldFreezeNumber,Megabit,UseState,ControlValveDate,SyncFlag,PriceSetNo,CollectorReadDate,MeterStatusString,Remnant from ReadNowData ");
			strSql.Append(" where ID=@ID");
			SqlParameter[] parameters = {
					new SqlParameter("@ID", SqlDbType.Int,4)
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public WebDemo.Model.WebDemo.ReadNowData DataRowToModel(DataRow row)
		{
			WebDemo.Model.WebDemo.ReadNowData model=new WebDemo.Model.WebDemo.ReadNowData();
			if (row != null)
			{
				...
				if(row["ControlValveDate"]!=null && row["ControlValveDate"].ToString()!="")
				{
					model.ControlValveDate=DateTime.Parse(row["ControlValveDate"].ToString());
				}
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select ID,MeterID,ReadNumber,ValveState,AttackState,Voltage,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,OldFreezeNumber,Megabit,UseState,ControlValveDate,SyncFlag,PriceSetNo,CollectorReadDate,MeterStatusString,Remnant ");
			strSql.Append(" FROM ReadNowData ");
			if(strWhere.Trim()!="")
			{
		...
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			...
			strSql.Append(" ID,MeterID,ReadNumber,ValveState,AttackState,Voltage,ReadDate,InstantNumber,PileNumber,Watertemp,BackWatertemp,WorkDate,FactDate,BatteryState,Warning,OldFreezeNumber,Megabit,UseState,ControlValveDate,SyncFlag,PriceSetNo,CollectorReadDate,MeterStatusString,Remnant ");
			strSql.Append(" FROM ReadNowData ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from ReadNowData T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "ReadNowData";
